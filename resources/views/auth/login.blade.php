...
<body class="bg-slate-200">
   <div class="flex flex-col md:flex-row mx-10 sm:mx-20 items-center relative justify-center min-h-screen">
      ...
      <div class="w-full md:w-1/2 lg:w-1/3 p-16 md:h-[56vh] bg-black bg-opacity-[0.45] relative md:order-2 shadow-2xl md:rounded-lg md:rounded-l-none flex flex-col gap-y-3 text-slate-100 overflow-hidden items-center justify-center text-center">
      ...
      <div class="w-full md:w-1/2 lg:w-1/3 p-5 bg-white shadow-2xl border md:rounded-lg md:rounded-r-none md:h-[56vh] md:flex md:flex-col md:justify-center">
         ...
         <form action="{{ route('login') }}" method="post" >
            ...
            <div class="mb-4">
               ...
               <input type="text" class="w-full px-4 py-2 rounded-lg focus:outline-none placeholder:text-slate-600/50 placeholder:text-sm text-slate-800 focus:ring form-normal-login @error('email') form-invalid @enderror" placeholder="Input Your Username" id="name" name="name" value="{{ old('email') }}">
               @error('email')
